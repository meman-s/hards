...
VALUES
    ...
    -- заказ 4: 2 стула
    (8, 5, 3, 1),
    -- заказ 5: 1 клавиатура
    (9, 6, 1, 1),
    -- заказ 6: 1 ноутбук
    (10, 6, 3, 1),
    -- заказ 6: 1 клавиатура
    (11, 7, 2, 3);
-- заказ 7: 3 мыши (отменен)
-- ============================================
-- ЗАДАЧИ
-- ============================================
...
SELECT
    ...
    avg(ct.order_sum) as avg_check
from
    client_total ct
GROUP BY
    ct.customer_name
having
    avg(ct.order_sum) > 15000
ORDER BY
    avg_check DESC;
-- SELECT
--     c.customer_name,
...
-- ВАШ КОД ЗДЕСЬ:
create
or replace function check_quantity() returns trigger as $$ BEGIN
    if new .quantity <= 0 THEN RAISE
    EXCEPTION
        'sldkf';
end if;
return new;
...
$$ LANGUAGE plpgsql;
create trigger trigger_check_quantity before
INSERT
    or
UPDATE
    on order_items for each row execute function check_quantity();
-- РЕШЕНИЕ ЗАДАЧИ 4:
-- SELECT
--     c.customer_name,
--     AVG(order_sum) as avg_check
